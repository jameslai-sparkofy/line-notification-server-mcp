...
export function registerAdminTools(server: Server, client: LineNotificationClient) {
  return {
    // 列出 Bot 帳號
    line_list_bots: {
      name: "line_list_bots",
      ...
      handler: async (args: { includeInactive?: boolean }) => {
    ...
    line_create_tag: {
      ...
      handler: async (args: { name: string; color?: string; category?: string; description?: string }) => {
    ...
    line_get_send_logs: {
      ...
      handler: async (args: { botId?: string; status?: string; limit?: number; startDate?: string; endDate?: string }) => {
        const { botId, ...query } = args;
        return client.get('/api/sendlogs', { botId, query: query as Record<string, string | number | boolean> });
      },
    },
